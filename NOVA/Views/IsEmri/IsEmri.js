    ...
    function rehberAc(element) {
        sonDLHamRowNum= $(element).parent().parent().parent().index()
           
        ShowProgress()
        axios.get('http://192.168.2.13:83/api/seri/serirehber').then(function (response) {
    ...
    const seriDL = document.getElementById('seriDL');
    ...
    function rechangeDL() {
        axios.get('http://192.168.2.13:83/api/seri/serirehber').then(function (response) {
            var b = response.data.filter(x => x.SERI_NO==seriDL.value)
            if (b != null) {
                var table = $('#DLHammaddeCard').find('table').eq(0);
                var currentRow = table.find('tr').eq(sonDLHamRowNum);
                currentRow.find('input').eq(4).val(b[0].ACIK2)
                currentRow.find('input').eq(5).val(b[0].ACIK1)
                currentRow.find('input').eq(10).val((b[0].MIKTAR * currentRow.find('input').eq(0).val()).toLocaleString('tr-TR') + " / " + (b[0].MIKTAR).toLocaleString('tr-TR'))
                currentRow.find('label').eq(0).text(b[0].MENSEI)
                currentRow.find('input').eq(3).val(b[0].MIKTAR2)
                currentRow.find('input').eq(7).val(b[0].SERI_NO_4)
                currentRow.find('input').eq(8).val(b[0].SERI_NO_3)
                currentRow.find('textarea').eq(0).val(b[0].STOK_ADI)
                var genislikalti = currentRow.find('input').eq(6).val() || 0;
                
                currentRow.find('input').eq(9).val(parseFloat((parseFloat(b[0].MIKTAR * currentRow.find('input').eq(0).val())) / ((parseFloat(b[0].ACIK1) + parseFloat(genislikalti)) * parseFloat((b[0].ACIK2).replace(",", ".")) * (0.00786))).toFixed(2) + " / " + parseFloat(parseFloat((b[0].MIKTAR)) / ((parseFloat(b[0].ACIK1) + parseFloat(genislikalti)) * parseFloat((b[0].ACIK2).replace(",", ".")) * (0.00786))).toFixed(2))
