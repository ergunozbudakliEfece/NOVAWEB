...
namespace NOVA.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> MailControl(Login login)
        {
            var userinfo = GetUserByMail(login.User.USER_MAIL);

            if (userinfo != null)
            {
                Session["Mail"] = login.User.USER_MAIL;
                Session["Guid"] = Guid.NewGuid();
                ViewBag.Info = "Link mail adresinize gönderilmiştir.";
                ...
                string u = "http://192.168.2.13/sifre/index?l=" + Encrypt(GetLink().ToString()) + "&us=" + Encrypt(userinfo.USER_NAME);
                string body = "Merhaba " + userinfo.USER_FIRSTNAME + " " + userinfo.USER_LASTNAME + ",</br></br>" +
                ...
                WebMail.Send(userinfo.USER_MAIL, subject, body, "sistem@efecegalvaniz.com", null, null, true, null, null, null, null, null, null);
