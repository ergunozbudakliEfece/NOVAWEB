...
class BSTable {
  ...
  constructor(tableId, options) {
      ...
      advanced: {                     // Do not override advanced unless you know what youre doing
          ...
          buttonHTML: `<div class="btn-group pull-right">
                <button id="bEdit" type="button" class="btn btn-sm btn-default">
                    <span class="bx bx-edit bx-md"> </span>
                </button>
                ...
                <button id="bAcep" type="button" class="btn btn-sm btn-default" style="display:none;">
                    <span class="bx bx-checkbox-checked bx-md"> </span>
                </button>
                <button id="bCanc" type="button" class="btn btn-sm btn-default" style="display:none;">
                    <span class="bx bxs-dizzy bx-md"> </span>
                </button>
            </div>`
      }
    };
    ...
    /** @private */ this.actionsColumnHTML = '<td class="bstable-actions" name="bstable-actions">' + this.options.advanced.buttonHTML + '</td>'; 
  ...
  // Indicate user is editing the row
    ...
    this._modifyEachColumn(this.options.editableColumns, $cols, function($td) {  // modify each column
      ...
      console.log(content);
        ...
        if ($td.attr('name') == 'datetime') {
            input = '<input class="form-control input-sm" type="datetime-local"  data-original-value="' + content + '"value="' + content.split('/')[2] + "-" + content.split('/')[1] + "-" + content.split('/')[0]+'T00:00">';
        }
    ...
    if ($allRows.length==0) { // there are no rows. we must create them
      ...
      $cols.each(function() {
        let column = this; // Inner function this (column object)
          ...
          } else if ($(column).attr('name') == 'bstable-select') {
              newColumnHTML = newColumnHTML + '<select class="form-select hep w-100"><option selected value="AD">AD</option> <option value="KG">KG</option><option value="MT">MT</option> <option value="M2">M2</option><option value="M3">M3</option><option value="PK">PK</option></select>';
      ...
      $cols.each(function() {
        let column = this; // Inner function this (column object)
          if ($(column).attr('name') == 'bstable-actions') {
              ...
              $(column).html('<select class="form-select hep" style="width:65%"><option selected value="AD">AD</option> <option value="KG">KG</option><option value="MT">MT</option> <option value="M2">M2</option><option value="M3">M3</option><option value="PK">PK</option></select>') ;
